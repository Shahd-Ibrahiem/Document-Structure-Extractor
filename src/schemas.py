...
class InvoiceExtraction(BaseModel):
    vendor_name: str = Field(description="Name of vendor or issuing company")
    invoice_number: Optional[str] = Field(description="Invoice ID or receipt number")
    invoice_date: Optional[str] = Field(description="Date invoice was issued")
    ...
    tax_amount: Optional[float] = Field(description="Tax amount charged")
    total_amount: float = Field(description="Final total bill amount")
    confidence_score: float = Field(description="Confidence rating between 0.0 and 1.0 based on document clarity")
    extraction_notes: List[str] = Field(description="Flags or ambiguities found during extraction")
